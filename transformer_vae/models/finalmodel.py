...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, pad_mask=None, causal=False):   # [B, T, H]
        ...
        if pad_mask is not None:
            valid = pad_mask[:, None, None, :]          # keys [B, 1, 1, T]
            query_valid = pad_mask[:, None, :, None]    # queries [B, 1, T, 1]
            mask = ~(valid & query_valid)  # [B, 1, T, T]
            attn_logits = attn_logits.masked_fill(mask, float('-inf'))

        if pad_mask is not None:
            attn = torch.where(
                query_valid,
                F.softmax(attn_logits / math.sqrt(self.d), dim=-1),
...
class VaeTransformer(nn.Module):
    ...
    def decode(self, z, x=None, mode='eval', sos_id=1):
        ...
        max_len = int(torch.round(pred_len.max()).item())

        if mode == 'train': 
            mask = (x != 0)
            h = torch.zeros(B, max_len, self.hidden_size, device=z.device)
            h[:, 0, :] = self.embedding.weight[sos_id]
            h += self.pos_encoder(h)
            

            h, _ = self.cross_block(h, slots, slots)    
            ...
            outputs = []
            for i in range(B):
                Li = max(1, int(torch.round(pred_len[i]).item())) 
                slots_i = slots[i:i+1]
                h = torch.zeros(1, Li, self.hidden_size, device=z.device)  
                h[:, 0, :] = self.embedding.weight[sos_id] 
                h += self.pos_encoder(h)     

                h, _ = self.cross_block(h, slots_i, slots_i)
...
def vae_loss(logits, x, mu, logvar, pred_len,
             beta=0.01, alpha=0.1, pad_id=0):
