...
# decoder transformers non regressive
class VaeTransformer(nn.Module):
    def __init__(self, vocab_size, hidden_size, latent_size, max_len, attn_heads=10, mask_token_id=0, seed_size=5):
        super().__init__()
        # Encoder
...
class VaeTransformer(nn.Module):
    def __init__(self, vocab_size, hidden_size, latent_size, max_len, attn_heads=10, mask_token_id=0, seed_size=5):
        ...
        self.max_len = max_len
        self.seed_size = seed_size
        self.fc_seed = nn.Linear(latent_size, hidden_size)
        # Self attention on masked sequences
        self.decoder = MultiHeadAttention(hidden_size, attn_heads)
        # output head
        self.fc_output = nn.Linear(hidden_size, vocab_size)
    ...
    def decode(self, z, sos_token_id=2, temperature=1.0):
        B, _ = z.shape
        device = z.device
        L, H = self.max_len, self.hidden_size
        vocab_size = self.fc_output.out_features

        # Tensor to store predicted tokens
        x = torch.full((B, L), sos_token_id, dtype=torch.long, device=device)
        # Tensor to store logits at every step
        logits_all = torch.zeros(B, L, vocab_size, device=device)

        z_emb = self.fc_seed(z).unsqueeze(1)  # [B, 1, H]

        for i in range(L):
            h = self.embedding(x[:, :i+1]) + z_emb  # [B, i+1, H]
            T = i+1
            causal_mask = torch.triu(torch.ones(T, T, device=device), diagonal=1).bool()
            h_dec = self.decoder(h, mask=causal_mask)  # [B, T, H]
            logits = self.fc_output(h_dec[:, -1, :])  # [B, vocab_size]
            probs = F.softmax(logits / temperature, dim=-1)
            next_token = torch.multinomial(probs, 1).view(-1)  # [B]
            x[:, i] = next_token
            logits_all[:, i, :] = logits  # save logits for this step

        return x, logits_all  # x: [B, L], logits_all: [B, L, V]
...
# Fast scnd version
class MultiHeadAttention_fst(nn.Module):
